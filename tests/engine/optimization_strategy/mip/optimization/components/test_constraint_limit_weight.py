...
def test__constraint_limit_weight__matches_expected_constraint(banana, chips):
    # ARRANGE
    request = Request(max_weight_kg=5.0, max_budget_usd=10.0, products=[banana, chips])
    expected_lhs = LinearExpression()
    expected_lhs.add(banana.weight_kg, Optimization.variable_name(banana.name))
    expected_lhs.add(chips.weight_kg, Optimization.variable_name(chips.name))
    expected = LinearConstraint(
        name="weight_limit", lhs=expected_lhs, sign=ConstraintSign.LEQ, rhs=request.max_weight_kg
    )

    # ACT
    constraint = ConstraintLimitWeight().build(request, name_fn=Optimization.variable_name)
